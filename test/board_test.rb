...
class BoardTest < Minitest::Test
 ...
 # -- build_board --
  ...
  def test_it_can_set_a_computer_ship
    board = Board.new
    board.build_board()
    grid_arr = ["A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4",
                "C1", "C2", "C3", "C4", "D1", "D2", "D3", "D4"]
    coords = board.set_computer_ship(3)

    assert coords.all?{ |coord| grid_arr.include?(coord)}

  end

  def test_it_can_find_a_random_coord_for_ship
    board = Board.new
    grid_arr = ["A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4",
                "C1", "C2", "C3", "C4", "D1", "D2", "D3", "D4"]
    assert grid_arr.include?(board.random_ship(2)[0])
    assert grid_arr.include?(board.random_ship(2)[0])
    assert grid_arr.include?(board.random_ship(2)[0])
    assert grid_arr.include?(board.random_ship(2)[0])
    assert grid_arr.include?(board.random_ship(2)[0])
  ...
  def test_it_can_random_fire
    board = Board.new
    board.build_board()
    grid_arr = ["A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4",
                "C1", "C2", "C3", "C4", "D1", "D2", "D3", "D4"]
    assert grid_arr.include?(board.random_fire)
    assert grid_arr.include?(board.random_fire)
    assert grid_arr.include?(board.random_fire)
    assert grid_arr.include?(board.random_fire)
    assert grid_arr.include?(board.random_fire)
